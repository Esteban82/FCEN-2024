...
# 	Nombre archivo de salida
	PENDIENTE=tmp_pendiente.nc
	ASPECTO=tmp_aspecto.nc
	PEND_ASPECTO=Pendiente_Aspecto.nc
...
#	Calcular Grillas con modulo del pendiente (-D -S) y aspecto (-Da)
	gmt grdgradient $DEM -R$REGION -fg -D  -S$PENDIENTE
	gmt grdgradient $DEM -R$REGION -fg -Da -G$ASPECTO
...
#	A=1 ; B=10 ; C=20
#	A=1 ; B=05 ; C=10
	gmt grdclip $PENDIENTE -G$PENDIENTE -Sb$A/10 -Si$A/$B/20 -Si$B/$C/30 -Sa$C/40

#	Reclasificar grilla de Aspecto en 8 clases 
#	En rangos de 45º en cada direccion (N, NE, E, SE, S, SO, O, NO)
	gmt grdclip $ASPECTO -G$ASPECTO -Sb22.5/1 -Si22.5/67.5/2 -Si67.5/112.5/3 -Si112.5/157.5/4 -Si157.5/202.5/5 \
	-Si202.5/247.5/6 -Si247.5/292.5/7 -Si292.5/337.5/8 -Sa337.5/1

#	-------------------------------------------------------------------------------
#	Sumar Grillas
	gmt grdmath $PENDIENTE $ASPECTO ADD = $PEND_ASPECTO

#	Crear Imagen
	gmt grdimage $PEND_ASPECTO -C$CPT.cpt
